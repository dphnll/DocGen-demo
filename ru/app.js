...
const renderContract = (data) => `
  ...
  <p>
    ...
    действующего на основании устава, именуемый в дальнейшем "Заказчик", с одной стороны,
    и ${token(data.contractorName)}, ИНН ${token(data.contractorInn)}, статус: ${token(data.contractorType)},
    именуемый в дальнейшем "Исполнитель", с другой стороны, заключили настоящий договор о нижеследующем.
  </p>
  <h4>1. Предмет договора</h4>
  <p>
    1.1. Исполнитель обязуется оказать Заказчику услуги: ${token(data.serviceDescription)}, а Заказчик обязуется принять результат услуг и оплатить их в порядке, установленном настоящим договором.
  </p>
  <p>
    1.2. Детальное описание услуг, сроки и результат работ фиксируются в Приложении N ${token(data.sowNumber)}, которое является неотъемлемой частью настоящего договора.
  </p>
  <h4>2. Сроки оказания услуг</h4>
  <p>
    2.1. Период оказания услуг: с ${token(data.startDateFormatted)} по ${token(data.endDateFormatted)}.
  </p>
  <p>
    2.2. Изменение сроков возможно по письменному согласованию сторон.
  </p>
  <h4>3. Стоимость и порядок оплаты</h4>
  <p>
    3.1. Стоимость услуг составляет ${token(`${data.amountFormatted} руб.`)}.
  </p>
  <p>
    3.2. Оплата производится на условиях: ${token(data.paymentTerms)}.
  </p>
  <p>
    3.3. Обязательство Заказчика по оплате считается исполненным с даты списания денежных средств со счета Заказчика.
  </p>
  <h4>4. Приемка услуг</h4>
  <p>
    4.1. По завершении оказания услуг стороны подписывают Акт оказанных услуг N ${token(data.actNumber)}.
  </p>
  <p>
    4.2. Если у Заказчика есть замечания к результату услуг, он направляет их Исполнителю в письменной форме в течение 5 рабочих дней.
  </p>
  <p>
    4.3. При отсутствии мотивированных замечаний услуги считаются оказанными надлежащим образом.
  </p>
  <h4>5. Ответственность сторон</h4>
  <p>
    5.1. Стороны несут ответственность за неисполнение или ненадлежащее исполнение обязательств в соответствии с законодательством и условиями настоящего договора.
  </p>
  <p>
    5.2. Исполнитель не отвечает за задержки, вызванные несвоевременным предоставлением Заказчиком информации, материалов или согласований.
  </p>
  <h4>6. Конфиденциальность</h4>
  <p>
    6.1. Стороны обязуются не раскрывать третьим лицам информацию, полученную в связи с исполнением настоящего договора, за исключением случаев, предусмотренных законом.
  </p>
  <h4>7. Заключительные положения</h4>
  <p>
    7.1. Настоящий договор вступает в силу с даты подписания и действует до полного исполнения сторонами обязательств.
  </p>
  <p>
    7.2. Все изменения и дополнения оформляются в письменной форме.
  </p>
  <h4>8. Реквизиты и подписи сторон</h4>
  <table class="requisites-table">
    <thead>
      <tr><th>Заказчик</th><th>Исполнитель</th></tr>
    </thead>
    <tbody>
      <tr>
        <td>
          ${token(data.customerName)}<br />
          ИНН: ${token(data.customerInn)}<br />
          Адрес: ${token(data.customerAddress)}<br />
          Подписант: ${token(data.customerSigner)}
        </td>
        <td>
          ${token(data.contractorName)}<br />
          ИНН: ${token(data.contractorInn)}<br />
          Контакт: ${token(data.contractorContact)}<br />
          Статус: ${token(data.contractorType)}
        </td>
      </tr>
      <tr>
        <td>Подпись: __________________ / ${token(data.customerSigner)}</td>
        <td>Подпись: __________________ / ${token(data.contractorName)}</td>
      </tr>
    </tbody>
  </table>
  <p class="muted-line">
    Динамические поля выделены цветом. После подтверждения карты полей шаблон настраивается под ваши реальные документы.
  </p>
`;
